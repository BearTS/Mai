...
const baseURI = 'https://rra.ram.moe/i/r?type=pat'
  ...
  detectUserfromArgs(message).then(mentions => {
    ...
    fetch(baseURI).then(res=>res.json()).then(json => {
      ...
      if (mentions.length<1){
        embed.setDescription(`You can't pat yourself, let me pat you instead, ${message.member}!`)
        return message.channel.send(embed)
      } else if (mentions.includes(`**me**`)){
        embed.setDescription(`${message.member} pats ${stringifyMentions(mentions)}!`)
        return message.channel.send(embed).then(()=>{
           message.channel.send(`H-Haa.. (✿´ ꒳ \` ) please don't stop...`)
        })
      } else {
        embed.setDescription(`${message.member} pats ${stringifyMentions(mentions)}!`)
        return message.channel.send(embed)
...
module.exports.help = {
  ...
  alias: ["headpat",'pet'],
  group: 'action',
  description: 'Pats the user/s you mentioned on the head!',
  examples: ['pat @MaiSakurajima MaiSakurajima','headpat MaiSakurajima','pet @MaiSakurajima'],
