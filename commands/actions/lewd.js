  ...
  detectUserfromArgs(message).then(mentions => {
    ...
    fetch(baseURI).then(res=>res.json()).then(json => {
      ...
      if (mentions.length<1){
        embed.setDescription(`${message.member}, kono ecchi!`)
        return message.channel.send(embed)
      } else if (mentions.includes(`**me**`)){
        embed.setDescription(`${message.member} says ${stringifyMentions(mentions)} ${(mentions.length===1) ? 'is' : 'are'} lewd?`)
        return message.channel.send(embed).then(()=>{
           message.channel.send(`Im not lewd!!!!`)
        })
      } else {
        embed.setDescription(`${message.member} says ${stringifyMentions(mentions)} ${(mentions.length===1) ? 'is' : 'are'} lewd?`)
        return message.channel.send(embed)
...
module.exports.help = {
  ...
  examples: ['lewd @MaiSakurajima MaiSakurajima','lewd MaiSakurajima','lewd @MaiSakurajima'],
