...
module.exports = {
  ...
  run: (client, message, [amount]) => profile.findById(message.author.id, (err,doc) => {
    ...
    if (err){
      ...
      return message.channel.send(`\\❌ **${message.member.displayName}**, You don't have a **bank** yet! Won bets might be higher than wallet capacity depending on your bet amount.\nGet a bank first by typing \`${client.prefix}bank\`. You must have at least **2,500** coins to register to a bank!`);
    } else if (isNaN(amount)){
      return message.channel.send(`\\❌ **${message.member.displayName}**, Please enter a valid amount.\nBets must be greater than **499** coins but less than **5,001**`);
    } else if (amount < 500 || amount > 5000){
      return message.channel.send(`\\❌ **${message.member.displayName}**, Amount is out of range. \nBets must be greater than **499** coins but less than **5,001**`);
      ...
      .then(async () => {
        ...
        if (!won){
          return message.channel.send(`\\❌ **${message.member.displayName}**, You lost **${text.commatize(amount)}** coins from your previous bet!\nYou can get more reliable coins without using the bet command!`);
        };
