...
module.exports = {
  ...
  adminOnly: true,
  group: 'setup',
  description: 'Set up the goodbye message. Supports Embeds!',
  examples: ['[Learn how to set up your Leaving Member Announcer](https://mai-san.ml/)'],
  parameters: [],
  run: async (client, message, [stats, ...args]) => {
    ...
    //sets the goodbye message to default
    if (stats === 'default'){
      data.goodbyeUse = 'default'
      return data.save().then(data => {
        profile.goodbye.use = data.goodbyeUse
        return message.channel.send(
          new MessageEmbed().setDescription(
    ...
    if (stats === 'embed=true'){
      if (!data.goodbyeEmbed || isEmpty(data.goodbyeEmbed))
      return message.channel.send(
        new MessageEmbed().setDescription(
          '<:cancel:712586986216489011>\u2000\u2000|\u2000\u2000'
    ...
    if (stats === 'msg=set'){
      ...
      data.goodbyemsg = args.join(' ')
      return data.save().then(data => {
        profile.goodbye.message = data.goodbyemsg
        return message.channel.send(
          new MessageEmbed().setDescription(
            '<a:animatedcheck:758316325025087500>\u2000\u2000|\u2000\u2000'
    ...
    if (stats === 'embed=set'){
      ...
      const newData = new MessageEmbed(data.goodbyeEmbed || {})
      ...
      data.goodbyeEmbed = newData

      return data.save()
      .then(data => {
        profile.goodbye.embed = data.goodbyeEmbed
        return message.channel.send(
          new MessageEmbed().setDescription(
            '<a:animatedcheck:758316325025087500>\u2000\u2000|\u2000\u2000'
   ...
   if (!profile.goodbye.enabled) //disabled channel
   return message.channel.send(
     new MessageEmbed().setDescription(
       '<:cancel:712586986216489011>\u2000\u2000|\u2000\u2000'
   ...
   if (!profile.goodbye.channel || !message.guild.channels.cache.get(profile.goodbye.channel)) //No channel detected
   return message.channel.send(
     new MessageEmbed().setDescription(
       '<:cancel:712586986216489011>\u2000\u2000|\u2000\u2000'
   ...
   if (!profile.goodbye.use) //use not set
   return message.channel.send(
     new MessageEmbed().setDescription(
       '<:cancel:712586986216489011>\u2000\u2000|\u2000\u2000'
