...
module.exports.run = async ( client, message, [user, time, scope]) => {
  ...
  if (scope && scope !== '-l') {
    ...
    if (scope === '-g') {
      ...
      const s = await message.channel.overwritePermissions([{id: member.id, deny: ['SEND_MESSAGES','ADD_REACTIONS']}]).catch(()=>{})
  ...
  message.channel.send(success(`Muted ${member} ${scope && scope === '-g' ? '[Globally] (Server-wide)' : '[Locally] (Channel-limited)' } for ${time} minutes!`))

  setTimeout(async () => {

    if (isMutedlocal(member,message) && scope !== '-g'){

      const y = await message.channel.overwritePermissions([{id: member.id, allow: ['SEND_MESSAGES','ADD_REACTIONS']}]).catch(()=>{})
