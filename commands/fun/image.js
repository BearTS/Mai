...
module.exports = {
  ...
  run: async (client, message, [ category ]) => {
    ...
    if (!['bird','cat','dog','lizard','booette','kaede','mai','moe','emilia','kanna','rem','booru'].includes(category.toLowerCase()))return message.channel.send(error(`Invalid category. Type \`${prefix}image categories\` to view available categories.`))
