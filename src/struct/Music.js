...
module.exports = class MusicPlayer extends Player {
  ...
  init(){
    const eventdir = join(__dirname, '..', 'util/player');
    const events = readdirSync(eventdir);

    for (const file of events.filter(f => f.split('.').pop() === 'js')){
      const event = require(join(eventdir, file));
      this.client.music.on(file.split('.')[0], event.bind(null, this.client));
    };
  };
